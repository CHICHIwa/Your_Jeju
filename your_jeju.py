...
indices_2 = load_model("Jeju/indices_2.pkl")
cosine_sim_2 = load_model("Jeju/cosine_sim_2.pkl")
final_review = load_model("Jeju/final_review.pkl")
tfidf_matrix_2 = load_model("Jeju/tfidf_matrix_2.pkl")
tfidf_2 = load_model("Jeju/tfidf_2.pkl")
...
def get_recommendations_by_user_input_with_sector(user_input, sector_name, tfidf_model, cosine_sim=cosine_sim_2):
    # 구역에 부합하는 행들 필터링
    sector_indices = final_review[final_review['구역'] == sector_name].index

    # TF-IDF 벡터 생성
    user_tfidf_vector = get_user_input_vector(user_input, tfidf_model)

    # 사용자 입력과 구역 필터링을 고려한 코사인 유사도 계산
    cosine_sim_user = linear_kernel(user_tfidf_vector, tfidf_matrix_2[sector_indices])
    ...
    # 상위 5개 식당 추출
    sim_scores = sim_scores[:5]
    restaurant_indices = [sector_indices[i[0]] for i in sim_scores]

    # 추천 식당과 유사도 반환
    recommended_restaurants = final_review.iloc[restaurant_indices][['식당명', '검색량합계값', '숙박_식당 거리']]
    similarity_scores = [round(i[1], 3) for i in sim_scores]

    return recommended_restaurants, similarity_scores

# 사용자에게 식당 추천하는 함수
def recommend_restaurant_city_downtown():
    st.subheader('> 제주시 + 서귀포시')
    unique_sector = set(final_review['구역'].values)
    
    user_sector = st.selectbox("제주시/서귀포시 中 어느 쪽 식당을 찾고 계신가요?", sorted(unique_sector))

    if not user_sector:
        st.warning("구역을 선택해주세요.")
        return

    user_input = st.text_input("어떤 식당을 찾으시나요? ")

    # 호텔과 사용자 입력에 기반한 식당 추천 및 유사도 가져오기
    recommended_restaurants, similarity_scores = get_recommendations_by_user_input_with_sector(user_input, user_sector, tfidf_2, cosine_sim_2)

    if recommended_restaurants.empty:
        #print("입력하신 조건에 부합하는 식당이 없습니다.")
        st.write("입력하신 조건에 부합하는 식당이 없습니다.")
    elif user_sector and user_input:
        with st.container():
            st.info("입력하신 조건과 호텔에 부합하는 식당을 아래와 같이 추천드립니다:")
            for idx, (restaurant, search_count, distance) in enumerate(recommended_restaurants.values):
                distance = round(distance, 2)
...
def show_pages(pages):
    for page in pages:
        if isinstance(page, Page):
            ...
            elif page.title == '제주도의 미래 소비 예측을 위한 Prophet':
            ...
            elif page.title == '식당 추천시스템_제주도':
                recommend_restaurant_city_downtown()                                                           
